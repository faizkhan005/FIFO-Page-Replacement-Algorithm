...
int count = 0;
BOOLEAN leftRotation, rightRotation;
...
void usleepx(unsigned int mseconds)
{
...
void drawNode(struct node* t_root, float x1, float y1, int level)
{
    ...
    if (t_root == NULL)
        ...
        sprintf_s(buff, "%d", t_root->key); //atoi
...
{
    int i;
    char buffer[500] = "What is an AVL tree?. ";
    glColor3f(0.0, 0.0, 0.0);
    ...
    strncpy_s(buffer, 50, "1.AVL tree is a self-balancing binary search tree", 49);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, buffer[i]);


    strncpy_s(buffer, 30, "2. The heights of the two child subtrees of ", 29);
    glRasterPos3f(-11, -9, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, buffer[i]);

    strncpy_s(buffer, 30, " any node differ by at most one ", 29);
    glRasterPos3f(-11, -11, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, buffer[i]);

    strncpy_s(buffer, 50, "3. If at any time they differ by more than one,", 29);
    glRasterPos3f(-11, -15, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, buffer[i]);

    strncpy_s(buffer, 50, "     rebalancing is done to the tree ", 29);
...
{
    int i;
    char buffer[500] = "Project By ";
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos3f(-5, -0.5, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, buffer[i]);

    strncpy_s(buffer, 30, "Punya, Madhuri", 29);
    glRasterPos3f(-5, -7, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, buffer[i]);


    strncpy_s(buffer, 30, "and Deepakshi ", 29);
...
{
    int i;
    char buffer[500] = "AVL TREE CONSTRUCTION ";
    glColor3f(0.0, 0.0, 0.0);
    ...
    strncpy_s(buffer, 30, "1. START", 29);
    glRasterPos3f(-5, -5.5, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, buffer[i]);

    strncpy_s(buffer, 30, "2. ABOUT", 29);
    glRasterPos3f(-5, -11, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, buffer[i]);

    strncpy_s(buffer, 30, "3. CREDITS", 29);
    glRasterPos3f(-5, -16.5, 1.5);
    for (i = 0; buffer[i] != '\0'; i++)
        glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, buffer[i]);

    strncpy_s(buffer, 30, "4. EXIT", 29);
